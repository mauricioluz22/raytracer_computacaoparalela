...
class camera {
  ...
  public:
    ...
    // sem mestre interno. Cada linha eh independente, entao nao ha regiao critica.
    // 'buffer' deve ter capacidade >= n_lines * image_width, armazenado linha a linha.
    void render_chunk(const hittable& world, int start_line, int n_lines, color *buffer) {
        if (!this->initialized) {
            this->initialize();
        }
        #pragma omp parallel for schedule(dynamic, 1)
        for (int row = 0; row < n_lines; row++) {
            int line = start_line + row;
            color *row_buffer = &buffer[row * image_width];
            for (int i = 0; i < image_width; i++) {
                ...
                row_buffer[i] = pixel_samples_scale * pixel_color;
            }
        }
    }
